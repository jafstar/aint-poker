...
function scoreHand(hand) {
  ...
  // Named card values for a richer description, e.g. "Two Pair, Jacks & Sevens"
  // instead of just the category — borrowed from the old repo's Winners
  // component pattern (handDesc), not just a bare hand name.
  const nameFor = (v) => RANKS[v - 2].length > 1 || v >= 11 ? { 11: 'Jacks', 12: 'Queens', 13: 'Kings', 14: 'Aces' }[v] || `${v}'s` : `${v}'s`;

  if (isStraight && isFlush) return { rank: 8, tiebreak, name: 'Straight Flush', desc: `Straight Flush, ${nameFor(values[0])} high` };
  if (countVals[0][1] === 4) return { rank: 7, tiebreak, name: 'Four of a Kind', desc: `Four of a Kind, ${nameFor(+countVals[0][0])}` };
  if (countVals[0][1] === 3 && countVals[1]?.[1] === 2) return { rank: 6, tiebreak, name: 'Full House', desc: `Full House, ${nameFor(+countVals[0][0])} full of ${nameFor(+countVals[1][0])}` };
  if (isFlush) return { rank: 5, tiebreak, name: 'Flush', desc: `Flush, ${nameFor(values[0])} high` };
  if (isStraight) return { rank: 4, tiebreak, name: 'Straight', desc: `Straight, ${nameFor(values[0])} high` };
  if (countVals[0][1] === 3) return { rank: 3, tiebreak, name: 'Three of a Kind', desc: `Three of a Kind, ${nameFor(+countVals[0][0])}` };
  if (countVals[0][1] === 2 && countVals[1]?.[1] === 2) return { rank: 2, tiebreak, name: 'Two Pair', desc: `Two Pair, ${nameFor(+countVals[0][0])} & ${nameFor(+countVals[1][0])}` };
  if (countVals[0][1] === 2) return { rank: 1, tiebreak, name: 'One Pair', desc: `One Pair, ${nameFor(+countVals[0][0])}` };
  return { rank: 0, tiebreak, name: 'High Card', desc: `High Card, ${nameFor(values[0])}` };
}
...
export async function playHand(apiKey) {
  ...
  const results = PLAYERS.map(p => {
    ...
    return { name: p.name, hand: hands[p.name], handName: score.name, handDesc: score.desc, rank: score.rank, tiebreak: score.tiebreak };
  }).sort((a, b) => b.rank - a.rank || b.tiebreak - a.tiebreak);

  // Real tie detection — same rank AND same tiebreak means an actual split
  // pot, not just "whoever sorted first." The old repo's Winners component
  // was built with this case in mind from the start; this version wasn't,
  // until now.
  const top = results[0];
  const winners = results.filter(r => r.rank === top.rank && r.tiebreak === top.tiebreak);

  return { hands, board, rounds, results, winners, isSplitPot: winners.length > 1 };
}
